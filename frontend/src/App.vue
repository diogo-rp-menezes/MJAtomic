...
export default {
    ...
    methods: {
        ...
        async submitAudit() {
            ...
            try {
                ...
                if(res.ok) {
                    ...
                    const newTask = this.tasks.find(t => t.id === data.task_id);
                    if (newTask) this.selectTask(newTask);
                }
            } catch(e) { alert("Error submitting audit: " + e); }
            finally { this.creating = false; }
        },
        async executePlan(id) {
            alert("Task started automatically upon creation.");
        },
        async resumePlan(id) {
            this.resuming = true;
            try {
                alert("Resume not implemented in backend yet.");
        ...
        async initProject() {
            ...
            try {
                const res = await fetch(`${API_URL}/init-project`, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ project_name: this.arch.name, description: this.arch.desc, stack_preference: this.arch.stack, root_path: "./workspace" })
                });
                const data = await res.json();
        ...
        getTaskStatus(task) {
            ...
            if (task.steps.length === 0) return 'PLANNING';
        ...
        getStatusColor(s) { return {'FAILED':'text-red-500','RUNNING':'text-blue-400','COMPLETED':'text-green-500','PLANNING':'text-purple-400'}[s] || 'text-gray-500'; },
